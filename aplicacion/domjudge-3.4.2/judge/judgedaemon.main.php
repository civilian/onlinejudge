...
// NOTICE OF COPYRIGHT                                                   //
//                                                                       //
//        Online UV Judge for Moodle Judgehost Server                    //
 ...
 * 
 * @package   judgehost
 * @author Jaap Eldering, Thijs Kinkhorst,
 * Peter van de Werken, Tobias Werth, Nicky Gerritsen, Oscar Chamat
...
function usage() {
    ...
    "  -t       is when you are running the judge online tests\n\n";
    exit;
}

$options = getopt("dv:n:hVt");
...
setup_database_connection(isset($options['test']));
...
while (TRUE) {
    ...
    $numopen = $DB->q('VALUE SELECT COUNT(*) FROM submission
        ...
        AND probid IN (%As) AND submittime < %s AND valid = 1', $cid, $judgable_lang, $judgable_prob, $cdata['endtime']);
    ...
    if ($numopen) {
        ...
        $submitid = $DB->q('MAYBEVALUE SELECT submitid
                                ...
                                LIMIT 1', $cid, $judgable_lang, $judgable_prob, $cdata['endtime']);
    ...
    $row = $DB->q('TUPLE SELECT CEILING(time_factor*timelimit) AS maxruntime, 
                       ...
                       p.documentation_style AS documid, p.indentation_style AS indentid, 
...
function judge($mark, $row, $judgingid) {
    ...
    $cpuset_opt = "";
    //isset() has a bug, it accepts spaces without the -option
    if (isset($options['daemonid']) && $options['daemonid'] !== " ")
    ...
    $DB->q('UPDATE judging SET output_compile = %s
                WHERE judgingid = %i AND judgehost = %s', getFileContents($workdir . '/compile.out'), $judgingid, $myhost);
    ...
    if (($result = $EXITCODES[$retval]) == 'compiler-error') {
        ...
        foreach ($testcases as $tc) {
            ...
            $DB->q('INSERT INTO judging_run (judgingid, testcaseid, runresult,
                ...
                VALUES (%i, %i, %s, %f, %s, %s, %s)', $judgingid, $tc['testcaseid'], $runresults[$tc['rank']], $runtime, getFileContents($testcasedir . '/program.out'), getFileContents($testcasedir . '/compare.out'), getFileContents($testcasedir . '/error.out'));
    ...
    if ($result == NULL)
        error("No final result obtained");
    
    
    //Grade the source code style
    logmsg(LOG_INFO, "Grading indentation...");
    system("mkdir -p '$workdir/indent'", $retval);
    if ($retval != 0)
        error("Could not create '$workdir/indent'");
    
    $files_indent = array();
    foreach ($sources as $rank => $source) {
        $srcfile = "$workdir/indent/$source[filename]";
        $files_indent[] = "'$source[filename]'";
        if (file_put_contents($srcfile, $source['sourcecode']) === FALSE) {
            error("Could not create $srcfile");
        }
    }
    ...
    $f;
    /* t */
    logmsg(LOG_INFO, "Indenting files...");
    /* t end */
    //Grading styles
    $indent = $DB->q('MAYBEVALUE SELECT i.name AS indent
                        FROM indentation_style i
                        WHERE i.indentation_styleid = %s'
            , $row['indentid']);
    $indent = $indent === NULL ? 'default' : $indent;
    
    @chmod(LIBDIR . "/indentation/", 0755);
    //TODO: Include an special indent colunm in the indentation_style table from database to do this only when needed
    //cp -pPRl "
    $comand = "cp -fr " . LIBDIR . "/indentation/indent-racket.rkt '$workdir'";
    system($comand, $retval);
    if ($retval != 0)
        logmsg(LOG_INFO, "Can't copy important files to indent especial languages " . $retval);

    @chmod(LIBDIR . "/indentation/", 0700);

    //Is mandatory send one file at the time because the emacs macros used 
    //depends on other scripts that uses 1 file at the time
    foreach ($files_indent as $file) {
        //emacs -batch sample.rkt -l scheme -f default1
        $f_indent = " " . $workdir . "/indent/" . $file;
        $comand = " emacs -nw -batch" . $f_indent . " -l " .
                LIBDIR . "/indentation/" . $row['langid'] . ".el -f " . $indent;
        system($comand, $retval);
        //logmsg(LOG_INFO, $comand);
        if ($retval != 0)
            logmsg(LOG_INFO, "Could not properly indent the source code " . $retval);
        /* t end */
    }

    logmsg(LOG_INFO, "Checking files indentation...");
    $grade = 0.0;
    foreach ($files_indent as $file) {
        $comand = 'python ' . LIBDIR . '/indentation/checkDiff.py ' . $workdir . '/indent/' . $file . ' ' . $workdir . '/compile/' . $file .
                ' > ' . $workdir . '/tmpIndentation.out';
        system($comand, $retval);
        //    logmsg(LOG_INFO, $comand);
        if ($retval != 0)
            error("Could not properly grade the indentation for " . $file);
        $grade += getFileContents($workdir . '/tmpIndentation.out');
    }
    $grade = $grade / count($files_indent);
    $indGrade = $grade;

    $DB->q('UPDATE judging SET indentation_grade = %f
                WHERE judgingid = %i AND judgehost = %s', $grade, $judgingid, $myhost);
    
    logmsg(LOG_INFO, "Indentation graded...");
    //End Grading the source code style
    //Grade the source code documentation
    

    logmsg(LOG_INFO, "Grading documentation...");
    foreach ($files as $file) {
            $f .= " " . $workdir . "/compile/" . $file;
    }
    //take the options for especification
    $docum = $DB->q('MAYBEVALUE SELECT d.name AS docum 
                        FROM documentation_style d
                        WHERE d.documentation_styleid = %s', $row['documid']);
    $docum = $docum === NULL ? 'default' : $docum;

    $doc = $DB->q('MAYBETUPLE SELECT name, author, version, copyright, license, 
                       package, param, retrn, param_pub, return_pub, see
                       FROM documentation_style 
                       WHERE langid = %s AND name = %s', $row['langid'], $docum);
    $options = '';
    $options .= ($doc['author'] == 1) ? ' -author' : '';
    $options .= ($doc['version'] == 1) ? ' -version' : '';
    $options .= ($doc['copyright'] == 1) ? ' -copyright' : '';
    $options .= ($doc['license'] == 1) ? ' -license' : '';
    $options .= ($doc['package'] == 1) ? ' -package' : '';
    $options .= ($doc['param'] == 1) ? ' -param' : '';
    $options .= ($doc['retrn'] == 1) ? ' -return' : '';
//        dbg($options);
    //java DocumentationCheck -language=cpp -author -files pruebas/soloComentario pruebas/CPPFileNavigator.java pruebas/D.java pruebas/Tree.java pruebas/Node.java
    $comandDocu = "cd " . LIBDIR . "/documentation/ && java DocumentationCheck -language=" . $row[langid] . " " . $options . " -files " . $f .
            " > " . $workdir . '/documentation.out';
    system($comandDocu, $retval);
//    logmsg(LOG_INFO, $comandDocu);

    if ($retval != 0)
        error("Could not grade the sourcode documentation exitcode " . $retval);

    $grade = getFileContents($workdir . '/documentation.out') + 0.0;
    $docGrade = $grade;

    $DB->q('UPDATE judging SET documentation_grade = %f
                WHERE judgingid = %i AND judgehost = %s', $grade, $judgingid, $myhost);
    //End Grading the source code documentation
    logmsg(LOG_INFO, "documentation graded...");
    ...
    $DB->q('UPDATE judging SET endtime = %s
                WHERE judgingid = %i AND judgehost = %s', now(), $judgingid, $myhost);
...
function store_result($result, $row, $judgingid) {
    ...
    $DB->q('UPDATE judging SET result = %s
                WHERE judgingid = %i AND judgehost = %s', $result, $judgingid, $myhost);
    ...
    if (!dbconfig_get('verification_required', 0)) {
        $DB->q('INSERT INTO event (eventtime, cid, teamid, langid, probid,
                        ...
                        VALUES(%s, %i, %s, %s, %s, %i, %i, "problem judged")', now(), $cid, $row['teamid'], $row['langid'], $row['probid'], $row['submitid'], $judgingid);
        if ($result == 'correct') {
            ...
            $numcorrect = $DB->q('VALUE SELECT count(submitid)
                                              ...
                                              WHERE valid = 1 AND probid = %s AND teamid = %s', $row['probid'], $row['teamid']);
            if ($numcorrect == 0) {
