...
class local_onlinejudge_compilation_error_scheme_testcase extends local_onlinejudge_base_testcase {
...
//        ONLINEJUDGE_STATUS_ACCEPTED);
        ...
        $this->assertGreaterThanOrEqual(89, $indentation_grade);
        // documentation
        $documentation_grade = floor($row['documentation_grade'] * 100);
        $this->assertGreaterThanOrEqual(99, $documentation_grade);
        
        $this->assertEquals(judge_ideone::$status_judge[$row['result']], 
            ONLINEJUDGE_STATUS_RUNTIME_ERROR);//scheme is interpreted
        $submission_graded = $assignment->get_submission($this->students[0]->id);
        $this->assertGreaterThanOrEqual(38, $submission_graded->grade);
...
//                $onlinejudge_result->status == ONLINEJUDGE_STATUS_TIME_LIMIT_EXCEED) {}
        ...
        $submission_graded_compileimportance = 
            $assignment_compileimportance->get_submission($this->students[0]->id);
        //There is not compilation so that don't count
        $this->assertGreaterThanOrEqual($submission_graded_compileimportance->grade, 42);
