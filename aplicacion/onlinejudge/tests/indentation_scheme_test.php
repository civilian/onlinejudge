...
class local_onlinejudge_indentation_scheme_testcase extends local_onlinejudge_base_testcase {
    ...
    function test_indentation_grade() {
        ...
        WHERE name = 'default' AND langid = %s", $oj_mid->language);
        ...
        $this->assertTrue($old_style == $new_style);
...
'
        ...
        // Indentation
        // TODO: improve the implementation of the scheme indentator
        // The indentator for scheme leaves extra chars in the file
        // can be improved comparing the two buffer (indent vs not-indent)
        // and ignoring spaces
        $indentation_grade = floor($row_low['indentation_grade'] * 100);
        $this->assertEquals($indentation_grade, 69);//Adjust this values
            // when the indentator is improved
        
        $indentation_grade = floor($row_mid['indentation_grade'] * 100);
        $this->assertEquals($indentation_grade, 87);
        
        $indentation_grade = floor($row_perfect['indentation_grade'] * 100);
        $this->assertEquals($indentation_grade, 94);
        
        $indentation_grade = floor($row_not_compile['indentation_grade'] * 100);
        $this->assertEquals($indentation_grade, 69);
        
        $indentation_grade = floor($row_weird_files['indentation_grade'] * 100);
        $this->assertEquals($indentation_grade, 92);
        
        $submission_graded = $assignment_perfect_grade->get_submission($this->students[0]->id);
        $this->assertEquals($submission_graded->grade,89);
        $submission_graded = $assignment_low_grade->get_submission($this->students[0]->id);
        $this->assertEquals($submission_graded->grade,68);
