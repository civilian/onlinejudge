 ...
 * @author    Sunner Sun, Oscar Chamat
...
function xmldb_assignment_onlinejudge_upgrade($oldversion = 0) {
    ...
    if ($oldversion < 2013100200) {
        // onlinejudge savepoint reached
        upgrade_plugin_savepoint(true, 2013100200, 'assignment', 'onlinejudge');
    }
    if ($oldversion < 2014020200) {
        // Define field dom_submission to be added to assignment_oj_submissions
        $table = new xmldb_table('assignment_oj_submissions');
        $field = new xmldb_field('dom_submission', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0', 'task');
        // Conditionally launch add field ideonepass
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        // onlinejudge savepoint reached
        upgrade_plugin_savepoint(true, 2014020200, 'assignment', 'onlinejudge');
    }
    if ($oldversion < 2014040200) {
        // Define fields to be added to assignment_oj
        $table = new xmldb_table('assignment_oj');
        $field = new xmldb_field('testcases_grade_worth', XMLDB_TYPE_NUMBER, '20,10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0.6', 'compileonly');
        ...
        $table = new xmldb_table('assignment_oj');
        $field = new xmldb_field('documentation_grade_worth', XMLDB_TYPE_NUMBER, '20,10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0.2', 'testcases_grade_worth');
        // Conditionally launch add field
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        $table = new xmldb_table('assignment_oj');
        $field = new xmldb_field('indentation_grade_worth', XMLDB_TYPE_NUMBER, '20,10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0.2', 'documentation_grade_worth');
        // Conditionally launch add field
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        // onlinejudge savepoint reached
        upgrade_plugin_savepoint(true, 2014040200, 'assignment', 'onlinejudge');
    }
    if ($oldversion < 2014040400) {
        ...
        $field = new xmldb_field('cpuusage', XMLDB_TYPE_NUMBER, '20,10', XMLDB_UNSIGNED, null, null, null, 'infostudent');

        // Launch change of type for field stdout
        $dbman->change_field_type($table, $field);

        // onlinejudge savepoint reached
        upgrade_plugin_savepoint(true, 2014040400, 'assignment', 'onlinejudge');
    }
    if ($oldversion < 2014040500) {
        // Define field ideoneuser to be added to assignment_oj
        $table = new xmldb_table('assignment_oj'); //AQUI
        $field = new xmldb_field('compileimportance', XMLDB_TYPE_INTEGER, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, 1, 'compileonly');
        // Conditionally launch add field ideoneuser
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        // onlinejudge savepoint reached
        upgrade_plugin_savepoint(true, 2014040500, 'assignment', 'onlinejudge');
    }
