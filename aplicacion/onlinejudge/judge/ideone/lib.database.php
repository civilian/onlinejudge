...
//        $this->auditlog('contest', 2, 'bd', $query);
//        $this->auditlog('contest', 2, 'q', $format);
    ...
    public function connect() {
        ...
        if (mysqli_connect_error() || !$this->_connection) {
            $vals = array ('host' => $this->host, 'user' => $this->user, 'password' => $this->password,'database' => $this->database);
            throw new RuntimeException( get_string('judgehostdbexception', 'local_onlinejudge', $vals)
     ...
     */
    public function auditlog($datatype, $dataid, $action, $extrainfo = null, $username = null) {
        global $DBDOM;

        $DBDOM->q1('INSERT INTO auditlog
	        (logtime, cid, user, datatype, dataid, action, extrainfo)
	        VALUES(%s, %i, %s, %s, %s, %s, %s)', strftime('%Y-%m-%d %H:%M:%S'), 2, null, $datatype, $dataid, $action, $extrainfo);
    }
    
    public function db($var){
        $this->auditlog('contest', 2, 'debug', $var);
    }
    ...
    //TODO: OBLIG quitar este metodo.
    public function q1() { // queryf
        $this->connect();
