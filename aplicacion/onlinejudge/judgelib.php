...
function onlinejudge_get_languages() {
    ...
    //TODO: cambiar de estatico
//    $judgeclasses =  array('judge_uvdaemon');
...
function onlinejudge_get_compiler_info($language) {
    //TODO: seria interesante meter aca los compiladores
    $judgeclasses = onlinejudge_get_judge_classes();
...
//    $judgeclass = 'judge_' . onlinejudge_judge_name($task->language);
    $judgeclass = 'judge_ideone';//KISS se asume una sola clase de juez si se quieren poner los demás hay que dejar el metodo de arriba con otras modificaciones
    $judgeclasses = onlinejudge_get_judge_classes();
    if (!in_array($judgeclass, $judgeclasses)) {
...
function onlinejudge_delete_coursemodule($cmid) {
    global $DB;
    global $DBDOM;
    $DBDOM->db("onlinejudge_delete_coursemodule(");
